...
CREATE PROCEDURE [esb].[_EntradasProductoTerminadoIntegracionError]
	@mid BIGINT, 
	@syncCodigo INT,
...
BEGIN
    ...
    BEGIN TRY
		...
		FROM msg.EntradasProductoTerminado t
		WHERE
			t.mid = @mid
        ...
        WHEN NOT MATCHED BY TARGET THEN
            ...
            externalId,id,supplier,arrivalDate,concept
            )
            VALUES
            (
            tipo_cambio,estado_cambio,intentos,fecha_ultimo_pull,fecha_ultimo_push,sync_codigo,sync_mensaje,sync_exception,
        ...
        FROM msg.EntradasProductoTerminado_Lineas a
        INNER JOIN @t b ON
            1 = 1
        WHERE
            a.mid = @mid
		...
		FROM esb.EntradasProductoTerminado t
		INNER JOIN @t a ON
