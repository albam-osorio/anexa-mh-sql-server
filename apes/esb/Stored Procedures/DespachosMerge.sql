...
CREATE PROCEDURE [esb].[DespachosMerge]
AS
BEGIN
	SET NOCOUNT ON;
    ...
    BEGIN TRY
		...
		BEGIN
            ...
            cte_00 AS(
				SELECT DISTINCT
                    ...
                    ,COALESCE(a.fecha_ultimo_pull,GETDATE()) AS fecha_ultimo_pull
				FROM stage.Despachos a
            ),
            cte_dup AS(
				SELECT
            ...
            (
			    ...
			    FROM cte_00 a
                LEFT OUTER JOIN esb.Despachos b ON
        ...
        BEGIN
            ...
            (
				SELECT
					 b.externalId
					,b.sku
					,b.expectedShipmentDate
					,b.amount
				FROM #source a
				INNER JOIN stage.Despachos b ON
					b.externalId = a.externalId
            ),
			cte_01 AS
			(
            ...
            AND c.sku = a.sku
            AND c.expectedShipmentDate = a.expectedShipmentDate
            ...
            (
                SELECT
                    b.*
			    FROM cte_00 a
			    INNER JOIN esb.Despachos_Lineas b ON
                    b.externalId = a.externalId
            ),
			cte_03 AS
			(
				...
				AND b.sku = a.sku
				AND b.expectedShipmentDate = a.expectedShipmentDate
        ...
        BEGIN
            ...
            WHEN NOT MATCHED BY TARGET THEN 
                INSERT (externalId,id,fecha_ultimo_pull,sincronizado    ,sourceId,destinationId,cliente,agencia,BODEGA_ORIGEN,BODEGA_DESTINO)
                VALUES (externalId,'',fecha_ultimo_pull,CAST(0 AS BIT)  ,sourceId,destinationId,cliente,agencia,BODEGA_ORIGEN,BODEGA_DESTINO)
            ...
            MERGE esb.Despachos_Lineas AS t
			USING cte_00 AS s ON
				s.externalId = t.externalId
			AND s.sku = t.sku
            AND s.expectedShipmentDate = t.expectedShipmentDate
            WHEN MATCHED AND NOT
				(t.amount = s.amount) THEN
			    UPDATE SET
				    t.amount = s.amount
			WHEN NOT MATCHED BY TARGET THEN
				INSERT (externalId,sku,amount,expectedShipmentDate)
				VALUES (externalId,sku,amount,expectedShipmentDate)
        ...
        IF @REGISTROS_SINCRONIZADOS > 0 OR @REGISTROS_NO_SINCRONIZABLES > 0 BEGIN
            ...
            BEGIN
			    ...
			    (
				    SELECT
                        ...
                        ,s.dup_sku
                    FROM #source s
				    LEFT OUTER JOIN esb.Despachos t ON
					    t.externalId = s.externalId
                    WHERE
                        s.se_puede_sincronizar = 0
                ),
                ...
                (
                    SELECT
                    	...
                    	,CASE 
                         ...
                         WHEN a.dup = 1                         THEN 'EL DESPACHO, ESTA DUPLICADA.'
                         WHEN a.dup_sku = 1                     THEN 'EL DESPACHO, TIENE UNO O MAS SKU DUPLICADOS'
                         WHEN a.sincronizacion_pendiente = 1    THEN 'EL DESPACHO, TIENE PENDIENTE UNA SINCRONIZACION'
            ...
            ---------------------------------------------------------------------------------------------------------------
            BEGIN
                MERGE msg.Despachos AS t
                USING #msg AS s ON
                    0 = 1
                WHEN NOT MATCHED BY TARGET THEN
                    INSERT
                    (tipo_cambio,estado_cambio,intentos,fecha_ultimo_pull,fecha_ultimo_push,sync_codigo,sync_mensaje,sync_exception,externalId,id,sourceId,destinationId,cliente,agencia,BODEGA_ORIGEN,BODEGA_DESTINO)
                    VALUES
                    (tipo_cambio,estado_cambio,intentos,fecha_ultimo_pull,fecha_ultimo_push,sync_codigo,sync_mensaje,sync_exception,externalId,id,sourceId,destinationId,cliente,agencia,BODEGA_ORIGEN,BODEGA_DESTINO)
                OUTPUT INSERTED.externalId,INSERTED.mid
                INTO @m(externalId,mid);

                INSERT INTO msg.Despachos_Lineas
                    (mid,sku,match_sku,amount,expectedShipmentDate)
                SELECT
                     a.mid
                    ,b.sku
				    ,b.match_sku
