...
CREATE PROCEDURE [esb].[OrdenesProduccionMerge]
AS
BEGIN
	SET NOCOUNT ON;
    ...
    BEGIN TRY
		...
		BEGIN 
            ...
            cte_00 AS(
				...
				FROM stage.OrdenesProduccion a
            ),
            cte_dup AS(
				SELECT
                     a.externalId
            ...
            (
			    ...
			    FROM cte_00 a
                LEFT OUTER JOIN esb.OrdenesProduccion b ON
            ...
            FROM #source a
            INNER JOIN esb.OrdenesProduccion b ON
                b.externalId = a.externalId
            WHERE
        ...
        BEGIN
            ...
            (
				SELECT
					...
					,b.amount
				FROM #source a
				INNER JOIN stage.OrdenesProduccion b ON
					b.externalId = a.externalId
            ),
			cte_01 AS
            (
            ...
            (
                SELECT
                    b.*
			    FROM cte_00 a
			    INNER JOIN esb.OrdenesProduccion_Lineas b ON
                    b.externalId = a.externalId
            ),
            cte_03 AS
            (
        ...
        BEGIN
		    ...
		    MERGE esb.OrdenesProduccion AS t
            ...
            WHEN NOT MATCHED BY TARGET THEN 
                INSERT (externalId,id,fecha_ultimo_pull,sincronizado    ,supplier,arrivalDate)
                VALUES (externalId,'',fecha_ultimo_pull,CAST(0 AS BIT)  ,supplier,arrivalDate)
            ...
            MERGE esb.OrdenesProduccion_Lineas AS t
			USING cte_00 AS s ON
				s.externalId = t.externalId
			AND s.sku = t.sku
            WHEN MATCHED AND NOT
        ...
        IF @REGISTROS_SINCRONIZADOS > 0 OR @REGISTROS_NO_SINCRONIZABLES > 0 BEGIN
            ...
            BEGIN
			    ...
			    (
				    SELECT
                        ...
                        ,s.dup_sku
                    FROM #source s
				    LEFT OUTER JOIN esb.OrdenesProduccion t ON
					    t.externalId = s.externalId
                    WHERE
                        s.se_puede_sincronizar = 0
                ),
                ...
                (
                    SELECT
                    	...
                    	,CASE 
                         ...
                         WHEN a.dup = 1                         THEN 'LA ORDEN DE PRODUCCION, ESTA DUPLICADA.'
                         WHEN a.dup_sku = 1                     THEN 'LA ORDEN DE PRODUCCION, TIENE UNO O MAS SKU DUPLICADOS'
                         WHEN a.sincronizacion_pendiente = 1    THEN 'LA ORDEN DE PRODUCCION, TIENE PENDIENTE UNA SINCRONIZACION'
            ...
            BEGIN
                ...
                WHEN NOT MATCHED BY TARGET THEN
                    INSERT
                    (tipo_cambio,estado_cambio,intentos,fecha_ultimo_pull,fecha_ultimo_push,sync_codigo,sync_mensaje,sync_exception,externalId,id,supplier,arrivalDate)
                    VALUES
                    (tipo_cambio,estado_cambio,intentos,fecha_ultimo_pull,fecha_ultimo_push,sync_codigo,sync_mensaje,sync_exception,externalId,id,supplier,arrivalDate)
		...
		BEGIN
			...
			WHERE
				a.codigo = 'ORDENES_DE_PRODUCCION'
		END
