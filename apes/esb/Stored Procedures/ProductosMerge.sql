...
CREATE PROCEDURE [esb].[ProductosMerge]
AS
BEGIN
	SET NOCOUNT ON;
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            AND NOT
			    (
			         a.name			= b.name
                 AND a.color		= b.color
			     AND a.codigoColor	= b.codigoColor
			     AND a.talla		= b.talla
			     AND a.tipoProducto	= b.tipoProducto
			     AND a.coleccion	= b.coleccion
			     AND a.grupoProducto = b.grupoProducto
			     AND a.subGrupoProducto = b.subGrupoProducto
			     AND a.fabricante	= b.fabricante
			     AND a.temporada	= b.temporada
			     AND a.referencia	= b.referencia
			     AND a.modelo		= b.modelo
			     AND a.genero		= b.genero
                )
        END
        ...
        BEGIN
            ...
            WHEN MATCHED AND NOT
			    (
			         t.name			= s.name
                 AND t.color		= s.color
			     AND t.codigoColor	= s.codigoColor
			     AND t.talla		= s.talla
			     AND t.tipoProducto	= s.tipoProducto
			     AND t.coleccion	= s.coleccion
			     AND t.grupoProducto = s.grupoProducto
			     AND t.subGrupoProducto = s.subGrupoProducto
			     AND t.fabricante	= s.fabricante
			     AND t.temporada	= s.temporada
			     AND t.referencia	= s.referencia
			     AND t.modelo		= s.modelo
			     AND t.genero		= s.genero			
			    ) THEN
                UPDATE SET 
                     t.name = s.name
                    ,t.color = s.color
                    ,t.codigoColor = s.codigoColor
                    ,t.talla = s.talla
                    ,t.tipoProducto = s.tipoProducto
                    ,t.coleccion = s.coleccion
                    ,t.grupoProducto = s.grupoProducto
                    ,t.subGrupoProducto = s.subGrupoProducto
                    ,t.fabricante = s.fabricante
                    ,t.temporada = s.temporada
            ...
            WHEN NOT MATCHED BY TARGET THEN 
                INSERT (externalId,id,fecha_ultimo_pull,sincronizado    ,companyPrefix,name,reference,ean,color,codigoColor,talla,tipoProducto,coleccion,grupoProducto,subGrupoProducto,fabricante,temporada,referencia,modelo,genero)
                VALUES (externalId,'',fecha_ultimo_pull,CAST(0 AS BIT)  ,companyPrefix,name,reference,ean,color,codigoColor,talla,tipoProducto,coleccion,grupoProducto,subGrupoProducto,fabricante,temporada,referencia,modelo,genero)
        ...
        IF @REGISTROS_SINCRONIZADOS > 0 OR @REGISTROS_NO_SINCRONIZABLES > 0 BEGIN
            ...
            BEGIN
			    ...
			    (
				    SELECT
                        ...
                        ,s.dup
                    FROM #source s
				    LEFT OUTER JOIN esb.Productos t ON
					    t.externalId = s.externalId
                    WHERE
                        s.se_puede_sincronizar = 0
                ),
                ...
                (
                    SELECT
                    	...
                    	,CASE 
                         WHEN a.dup = 1                         THEN 'EL PRODUCTO, ESTA DUPLICADO.'
                         WHEN a.sincronizacion_pendiente = 1    THEN 'EL PRODUCTO, TIENE PENDIENTE UNA SINCRONIZACION'
            ...
            ---------------------------------------------------------------------------------------------------------------
            BEGIN
                MERGE msg.Productos AS t
                USING #msg AS s ON
                    0 = 1
                WHEN NOT MATCHED BY TARGET THEN
                    INSERT
                    (tipo_cambio,estado_cambio,intentos,fecha_ultimo_pull,fecha_ultimo_push,sync_codigo,sync_mensaje,sync_exception,externalId,id,companyPrefix,name,reference,ean,color,codigoColor,talla,tipoProducto,coleccion,grupoProducto,subGrupoProducto,fabricante,temporada,referencia,modelo,genero)
                    VALUES
                    (tipo_cambio,estado_cambio,intentos,fecha_ultimo_pull,fecha_ultimo_push,sync_codigo,sync_mensaje,sync_exception,externalId,id,companyPrefix,name,reference,ean,color,codigoColor,talla,tipoProducto,coleccion,grupoProducto,subGrupoProducto,fabricante,temporada,referencia,modelo,genero)
